...
class ModelTest : public testing::Test {
 /*protected:
  void SetUp() override {
    playerOneId = model.addPlayer(Player("Tyler", 0));
    playerTwoId = model.addPlayer(Player("Alex", 1));
    playerThreeId = model.addPlayer(Player("Mia", 2));
    playerFourId = model.addPlayer(Player("Jeff", 3));
  }
